...
def sortRegistered(gebizpath, regconpath):
    ...
    with open(regconpath) as oFile:
        ...
        for row in reader:
            ...
            if row['company_name'] not in regCon:
    ...
    with open(gebizpath) as gFile:
        ...
        for row in reader:
            ...
            if row['supplier_name'] in regCon:
                ...
                if row['supplier_name'] not in regConAwarded:
                    regConAwarded.append(row['supplier_name'])
            #Else if current row's supplier name is not found in the list of registered contractors (meaning unregistered)
                ...
                if row['supplier_name'] not in unregConAwarded:
                    unregConAwarded.append(row['supplier_name'])
            #Append the full row of data into masterGebiz
            masterGebiz.append(row)
    return {'gebizData':masterGebiz,'regConData':masterCon,'regCon':regCon,'regConAwarded':regConAwarded,'unregConAwarded':unregConAwarded}
